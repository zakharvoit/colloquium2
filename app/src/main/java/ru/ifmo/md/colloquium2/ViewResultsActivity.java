...
public class ViewResultsActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Person> results = (List<Person>) getIntent().getSerializableExtra(MainActivity.VOTE_RESULTS);

        Person[] arrayResults = new Person[results.size()];
        for (int i = 0; i < results.size(); i++) {
            arrayResults[i] = results.get(i);
        }
        resultsList.setAdapter(new PersonArrayAdapter(this, arrayResults));
    }
